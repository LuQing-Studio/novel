...
**最后更新**: 2026-01-24 (原地编辑、局部重写与章节导出)
...
### Phase 21: 原地编辑 + 选中文本局部重写 + 章节导出增强 - 2026-01-24

- **目标**:
  - 章节阅读页支持“原地编辑”，无需在页面下方打开编辑窗口
  - 支持选中文本进行 AI 局部重写/扩写/精简，提高修改效率
  - 导出支持按章节导出（单章/多章选择），方便发布单章到平台
- **完成内容**:
  - [x] 章节页在正文区域原地切换编辑模式（textarea），支持保存/取消并自动写入版本历史
  - [x] 新增选中文本 AI 处理：预设改写/扩写/精简等模式 + 自定义指令，返回建议后可一键应用
  - [x] 修复章节 PUT 更新逻辑（移除不存在的 `updated_at` 字段），编辑后同步章节字数与小说字数统计
  - [x] 新增“导出本章”按钮（章节页）
  - [x] 小说页导出支持勾选章节/按章号范围选择导出（仍保留导出全文）
  - [x] 优化章节列表 API：列表不再返回全量 `content`，降低长篇小说时的页面开销
- **Git 提交**:
  - `a2913dc`: feat: inline chapter edit and selective export
- **验证方式**:
  - 使用 agent-browser：章节页点击“原地编辑”可保存并更新字数；选中文本可触发 AI 建议并应用；导出本章/导出选中章节触发下载

### Phase 20: 章节生成扩展性与体验优化 - 2026-01-24

- **目标**: 解决章节生成的上下文膨胀与长文本等待体验问题，让长篇写作可以继续扩展
- **完成内容**:
  - [x] 生成接口不再全量拉取章节内容：用 `COUNT(*)` 计算章节号，仅取最近 N 章做前情提要
  - [x] 人物卡/世界观设定按相关性 Top-K 注入 prompt，避免 Token 膨胀与注意力分散
  - [x] 伏笔分层注入：待揭示 / 需铺垫 / 长期未回收（背景约束）
  - [x] 章节生成支持流式输出（NDJSON），前端实时预览，完成后自动落库并跳转章节页
  - [x] PostgreSQL 连接池在 dev 环境复用，减少 HMR 下的重复连接风险
- **相关文档**:
  - `docs/plans/2026-01-24-context-and-streaming-first-principles.md`
- **Git 提交**:
  - `fb5022c`: feat: stream chapter generation and shrink context
- **验证方式**:
  - 使用 agent-browser：注册/登录 → 创建小说 → 流式生成章节（实时预览）→ 自动落库 → 跳转阅读

### Phase 19: 登录注册与权限隔离 - 2026-01-24

- **目标**: 引入登录/注册与资源归属校验，防止用户访问他人小说数据
- **完成内容**:
  - [x] 新增用户与会话模型（`users`/`sessions`），基于 HttpOnly Cookie 的会话登录
  - [x] 新增登录/注册页面（`/login`、`/register`）与退出登录
  - [x] 小说与相关资源（章节/人物/伏笔/世界观/知识图谱/导出/版本）全部按当前用户隔离
  - [x] 未登录访问核心页面统一跳转到 `/login`
- **Git 提交**:
  - `beb9bd2`: feat: add auth and per-user access control
- **验证方式**:
  - 使用 agent-browser 注册两个账号：账号 2 无法访问账号 1 的小说详情（404），首页也只显示自己的小说列表
- **问题记录 (Scale)**:
  - **P1 / 开发环境阻塞**: 本地 `DATABASE_URL` 指向 `localhost:5432` 时可能连到“其他项目”的 PostgreSQL，导致出现 `relation "users" does not exist`
  - **根因**: `docker compose` 的 `novel-db-1` 未将 5432 映射到宿主机端口（容器内可用，但 `localhost:5432` 会命中宿主机已有 PG）
  - **临时处理**: 本地开发改为直连容器网络地址；后续需要制定统一 DB 策略（共享 PG / 每项目独立端口 / 不映射端口）

### 文档维护: 验证工具改为 agent-browser - 2026-01-23

- **完成内容**:
  - [x] 将开发规范中的验证工具从 Chrome DevTools MCP 切换为 agent-browser
  - [x] 更新验证步骤与移动端验收说明
- **Git 提交**:
  - `b287fea`: docs: switch verification to agent-browser
- **备注**: 后续功能验收统一使用 agent-browser（建议 `--headed` 模式）
...
- **完成内容**:
  ...
  - [x] 创建 10 个提示词文档文件
  - [x] 更新 CLAUDE.md 开发规范
  - [x] 配置 Claude Code hooks 自动提醒机制
- **关键决策**:
  - 区分知识库内容 (架构决策) 和日志内容 (bug/问题)
  - 按开发阶段组织提示词文档
  - 提取提示词模式和特点作为经验总结
  - 使用 hooks 机制强制 AI 遵循开发规范
  - 移除不适用的 project-kickstart skill 引用
- **技术细节**:
  - 创建 ~/.claude/hooks/check-claude-md.sh
  - 配置 SessionStart 和 UserPromptSubmit hooks
  - 更新 CLAUDE.md 添加醒目的 AI 必读提醒
  - 更新技术栈信息 (DeepSeek, 硅基流动, LightRAG)
- **Git 提交**: 26ae031
- **备注**: 项目知识沉淀完成，开发规范优化完成
...
## Phase 17: 3D 知识图谱修复 (2026-01-22 10:40)

**目标**: 修复 3D 知识图谱 API 和渲染问题

### 完成内容

#### 1. 问题诊断
- ✅ 发现数据库使用 UUID 而不是整数作为 novel_id
- ✅ 识别 API 500 错误根本原因：类型不匹配
- ✅ 发现 three-spritetext 导入方式错误
- ✅ 识别节点颜色值为 undefined 导致渲染失败
...
**关键代码** (`app/api/novels/[id]/knowledge-graph/route.ts:12-20`):
```typescript
// 验证 UUID 格式
...
#### 3. 3D 渲染修复
- ✅ 修复 three-spritetext 默认导出导入方式
- ✅ 添加节点颜色默认值（`#ffffff`）
- ✅ 确保所有节点都有有效颜色值

**关键代码** (`app/novels/[id]/knowledge-graph/page.tsx:149-156`):
```typescript
nodeThreeObject={(node: any) => {
  // 动态导入 SpriteText (默认导出)
  const SpriteText = require('three-spritetext').default || require('three-spritetext');
  const sprite = new SpriteText(node.label);
  sprite.color = node.color || '#ffffff';  // 添加默认值
  sprite.textHeight = 8;
  return sprite;
}}
```

#### 4. Chrome DevTools 测试验证
- ✅ 使用正确的 UUID 访问页面：`055c5074-5f71-40ce-a0ca-8a594a77f9eb`
- ✅ 验证 API 返回正确数据（1 个人物节点：李明）
- ✅ 确认 WebGL 正常初始化
- ✅ 确认 Canvas 正确渲染（1904x900）
- ✅ 验证无控制台错误
- ✅ 确认图例显示正确统计（人物1，世界观0，伏笔0）

### 技术要点

1. **UUID vs 整数 ID**
   - PostgreSQL 表使用 `uuid` 类型作为主键
   - 需要正则表达式验证格式
   - API 路由参数类型为 string，需要验证

2. **three-spritetext 导入**
   - CommonJS 默认导出需要通过 `.default` 访问
   - 使用 `require('three-spritetext').default` 获取构造函数
   - 兼容性处理：`|| require('three-spritetext')`

3. **Three.js 颜色解析**
   - Three.js 要求颜色值为有效字符串
   - undefined 或 null 会导致解析错误
   - 必须提供默认值确保渲染成功

### Git 提交
- **Commit Hash**: `2400cc5`
- **提交信息**: fix: 修复 3D 知识图谱 API 和渲染问题

### 测试结果
| 测试项 | 结果 | 说明 |
|--------|------|------|
| API UUID 验证 | ✅ | 正确拒绝非 UUID 格式 |
| API 数据返回 | ✅ | 返回 1 个人物节点 JSON |
| WebGL 初始化 | ✅ | Canvas 正常获取 WebGL 上下文 |
| 3D 渲染 | ✅ | 节点和标签正常显示 |
| 控制台错误 | ✅ | 无任何错误或警告 |
| 图例统计 | ✅ | 正确显示各类型节点数量 |

### 待优化项
1. 添加更多测试数据（世界观、伏笔）以展示完整图谱
2. 优化节点布局算法，提升多节点时的视觉效果
3. 添加节点点击选中的视觉反馈（高亮、描边）
4. 实现相机动画的缓动效果

### Phase 18: Obsidian 风格 3D 知识图谱增强 (2026-01-22 13:10)

**目标**: 在 Next.js App Router 下实现更接近 Obsidian Graph View 的 3D 图谱体验（力导向布局 + 标签 + 交互式旋转/缩放/点击聚焦 + 曲线连线）

#### 完成内容
- ✅ 使用 `r3f-forcegraph` + `@react-three/fiber` + `@react-three/drei`，完全客户端渲染（SSR 安全）
- ✅ 节点采用 CanvasTexture Sprite 标签（暗色胶囊标签 + 描边阴影），视觉更接近 Obsidian
- ✅ 连线支持弯曲（`linkCurvature` + `linkCurveRotation`），并根据 link key 稳定随机旋转，避免重叠
- ✅ Hover/选中节点高亮：邻接节点保留原色、其余节点降亮度；关联连线高亮并开启粒子流动
- ✅ 点击节点：相机缓动聚焦（OrbitControls target + camera position 插值），用于"点击聚焦"
- ✅ 点击空白：取消选中（详情面板回到引导态）
- ✅ 增强 API：边去重 + 空 title/空 name 安全处理，避免重复连线与低质量匹配

#### 关键代码
- `app/novels/[id]/knowledge-graph/Graph3DClient.tsx`: 节点 Sprite 标签、曲线连线、高亮逻辑、相机聚焦缓动、空白点击取消
- `app/novels/[id]/knowledge-graph/KnowledgeGraphClient.tsx`: 客户端数据加载、图例/操作提示、详情面板、自动旋转开关
- `app/api/novels/[id]/knowledge-graph/route.ts`: 边去重与健壮性增强

#### Chrome DevTools 验证
- ✅ 本地 dev server 重新启动（释放 3000 端口后启动）
- ✅ 访问 `/novels/055c5074-5f71-40ce-a0ca-8a594a77f9eb/knowledge-graph`
- ✅ 验证 WebGL 渲染正常（Canvas 可见、节点标签可见）
- ✅ 点击"自动旋转"可切换为"停止旋转"
- ✅ 点击主题切换，亮/暗主题下页面布局与交互正常
- ✅ Console 无 error/warn

#### Git 提交
- **Commit Hash**: `e72a5b7`
- **提交信息**: feat: 实现 Obsidian 风格 3D 知识图谱增强
