...
interface ExistingChapterByNumberRow {
  id: string;
  planId: string | null;
}
...
export async function POST(
  ...
  try {
    ...
    const existingByNumber = await queryOne<ExistingChapterByNumberRow>(
      'SELECT id, plan_id FROM chapters WHERE novel_id = $1 AND number = $2',
      [id, chapterNumber]
    );
    if (existingByNumber?.planId && existingByNumber.planId !== planId) {
      return NextResponse.json(
        { error: 'Chapter number already exists and is bound to a different plan', existingPlanId: existingByNumber.planId },
        { status: 409 }
      );
    }
    ...
    async function persistChapter(content: string): Promise<Chapter> {
      ...
      const chapter =
        existingByNumber?.id
          ? await (async () => {
              const prev = await queryOne<{ id: string; content: string; wordCount: number | null }>(
                'SELECT id, content, word_count FROM chapters WHERE id = $1 AND novel_id = $2',
                [existingByNumber.id, id]
              );
              if (!prev) {
                throw new Error('Existing chapter not found when attempting overwrite');
              }

              const maxVersionRow = await queryOne<{ maxVersion: number }>(
              ...
              await query(
                ...
                [
                  ...
                  'system',
                  `Regenerated by plan ${planId}`.slice(0, 200),
                ]
              );

              const [updated] = await query<Chapter>(
                `UPDATE chapters
                 SET volume_id = $1,
                     plan_id = $2,
                     title = $3,
                     ...
                     outline = $5,
                     word_count = $6
                 WHERE id = $7 AND novel_id = $8
                 RETURNING *`,
                [confirmedPlan.volumeId, planId, chapterTitle, trimmed, outline, wordCount, prev.id, id]
              );
              return updated;
            })()
          : (
